...
def run_pool(items, callback=None, config=None):
    ...
    heads = tuple(
            # astart(x, ... )
            (x, callback, queue, config) for x in items
        )
    heads += ( (aproc_queue_handler, queue, callback), )
...
def aproc_queue_handler(queue, callback=None):
    print('aproc_queue_handler start')
    return asyncio.run(async_proc_queue_handler(queue, callback))


async def async_proc_queue_handler(queue, callback=None):
    q = queue
...
def astart(watch_dir, callback=None, queue=None, config=None):
    ...
    callback = partial(queue_put, queue)
    return asyncio.run(
        afunc(watch_dir, callback=callback, queue=queue, config=config))


def afunc(watch_dir, callback=None, queue=None, config=None):
    print('start', watch_dir)
    # queue = queue or Queue()
    callback = partial(queue_put, queue)
    return start(watch_dir, config=config, callback=callback, queue=queue)
